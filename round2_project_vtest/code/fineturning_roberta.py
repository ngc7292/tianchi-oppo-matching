...
def run():
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(train_samples, train_labels)):
        ...
        models.append(best_model.state_dict())
    ...
    for key,values in model.state_dict().items():
        ...
        for model_temp in models:
            if key in model_temp:
                temp.append(model_temp[key])
            para = temp[0]
            for i in temp[1:]:
                para += i
            model_dict[key] = para/3

    model.load_state_dict(model_dict)
    model.to(device)
    onnx_file = os.path.join(onnx_path, f"fold-roberta.onnx")
