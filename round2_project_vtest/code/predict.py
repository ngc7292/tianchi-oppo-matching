...
def tccapi():
    ...
    for i in range(len(index_list)):
        ...
        try:
            ...
            predict = infer(session_1, session_2,session_3,  tokenizer, query_A, query_B)
...
# 需要根据模型类型重写
def infer(session_1, session_2, session_3, tokenizer, text_1, text_2):
    ...
    output_1, output_2, output_3 = session_1.run(None, ort_inputs)[0], session_2.run(None, ort_inputs)[0], session_3.run(None, ort_inputs)[0]
    ...
    output_score_3 = np.exp(output_3) / (np.exp(output_3).sum(axis=1, keepdims=True))
    result_3 = output_score_3[:, 1] / output_score_3.sum(axis=1)

    result = (result_1[0] + result_2[0] + result_3[0]) / 3
...
def init_model(tokenizer_path):
    ...
    return tokenizer, session_1, session_2, session_3


if __name__ == "__main__":
    tokenizer_path = "../origin_model/tokenizer"
    tokenizer, session_1, session_2, session_3 = init_model(tokenizer_path)
