...
def run():
    ...
    model_name_or_path = "/remote-home/zyfei/project/tianchi/model_output/electra_output/checkpoint-30000"
    tokenizer = BertTokenizer(vocab_file=vocab_file)
    config = ElectraConfig.from_pretrained(model_name_or_path, num_labels=2, output_hidden_states=True)
    ...
    for i, (train_index, val_index) in enumerate(skf.split(train_texts_aug, train_labels_aug)):
        ...
        # model init is here
        model = ElectraForSequenceClassification.from_pretrained(model_name_or_path, config=config)
        model.to(device)
