...
def run():
    ...
    vocab_file = "../user_data/tmp_data/raw_data/electra_raw_text_ngram.txt"

    epochs = 5
    lr = 1e-5
    batch_size = 128
    n_split = 5
    fold_path = "../user_data/model_data/finturn_models/kfold-16"
    ...
    for i, (train_index, val_index) in enumerate(skf.split(train_texts_aug, train_labels_aug)):
        ...
        model = ElectraForSequenceClassificationWithClsCat.from_pretrained(model_name_or_path, config=config)
        model.to(device)
