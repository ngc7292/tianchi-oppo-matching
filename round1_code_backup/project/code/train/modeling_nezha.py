...
class NeZhaForSequenceClassificationWithClsCat(NeZhaPreTrainedModel):
    ...
    Examples::
        ...
        pooled_output = torch.tensor([], device=self.device)
        ...
        for i in range(1, len(all_hidden_states)):
            cls_state = all_hidden_states[i]
            cls_state = self.poolers[i-1](cls_state)
            # cls_state = cls_state[:, 0, :]
...
# not use this model class
class NeZhaForSequenceClassificationWithClSCat_2(NeZhaPreTrainedModel):
    def __init__(self, config, is_co_ocurrence=False):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = NeZhaModel(config, is_co_ocurrence=is_co_ocurrence)
        self.dropout = nn.Dropout(config.classifier_dropout_prob)
        # self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.classifier = nn.Linear(config.hidden_size * 24, 2048)
        self.classifier_2 = nn.Linear(2048, 2)
    ...
    Examples::
        ...
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier_2(self.classifier(pooled_output))
...
class NeZhaForSequenceClassificationWithClsAdd(NeZhaPreTrainedModel):
    def __init__(self, config, is_co_ocurrence=False):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = NeZhaModel(config, is_co_ocurrence=is_co_ocurrence)
...
class NeZhaForSequenceClassificationWithMutilDropout(NeZhaPreTrainedModel):
    def __init__(self, config, is_co_ocurrence=False, dropout_num=8):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = NeZhaModel(config, is_co_ocurrence=is_co_ocurrence)
        # mutil sample dropout
