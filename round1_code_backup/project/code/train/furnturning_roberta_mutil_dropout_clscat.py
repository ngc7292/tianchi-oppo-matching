...
class PGD():
    ...
    def attack(self, epsilon=0.5, alpha=0.3, emb_name='word_embeddings.', is_first_attack=False):
        # epsilon = 1. alpha = 0.3
        # emb_name这个参数要换成你模型中embedding的参数名
        for name, param in self.model.named_parameters():
            if param.requires_grad and emb_name in name:
...
def run():
    ...
    vocab_file = "../user_data/tmp_data/raw_data/roberta_raw_text_ngram.txt"

    epochs = 5
    lr = 1e-5
    batch_size = 64
    n_split = 5
    fold_path = "../user_data/model_data/finturn_models/kfold-14"
    ...
    # model_name_or_path = "/remote-home/zyfei/project/tianchi/model_output/roberta_output/checkpoint-20000"
    model_name_or_path = "/remote-home/zyfei/project/tianchi/models/roberta"
    tokenizer = BertTokenizer.from_pretrained(model_name_or_path)
    config = BertConfig.from_pretrained(model_name_or_path, num_labels=2, output_hidden_states=True)
    ...
    for i, (train_index, val_index) in enumerate(skf.split(train_texts_aug, train_labels_aug)):
        ...
        model = BertForSequenceClassificationClsCat.from_pretrained(model_name_or_path, config=config)
        model.to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=lr)  # adamW
